...
function LongTermFeat() {
    ...
    const secondRowCards = cardData.slice(3, 5);

    return (
        <section className="w-full bg-base mx-auto gap-[48px] px-6 md:px-[120px] py-20 flex flex-col items-center">
            <div className="flex flex-wrap justify-center gap-[48px] md:gap-12 w-full max-w-[1200px]">
                {firstRowCards.map((card, index) => (
                    <Card
                        key={index}
                        className="h-[250px] bg-base rounded-2xl shadow-[6px_7px_25.2px_#00000012] border-none w-full sm:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1rem)] max-w-[350px]"
                    >
                        <CardContent className="h-full p-[22px] flex flex-col items-center justify-center gap-2">
                            {card.icon}
                            <h3 className="font-gilroy text-base-black text-[28px] text-center leading-[31.5px]">
                                {card.title}
                            </h3>
                            <p className=" text-base-black text-center leading-[19.2px]">
                                {card.description}
                            </p>
                        </CardContent>
                    </Card>
